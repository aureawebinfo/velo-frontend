...
/* ============================================================================
   VELO — BALLPIT
   Puerto directo del componente de React Bits (mismo motor físico/WebGL),
   sin cambios de lógica — solo TS con @ts-nocheck porque el código fuente
   usa campos privados (#x) minificados que no vale la pena re-tipar.
   ============================================================================ */
...
class x {
  ...
  #p() {
    ...
    if (!this.renderer || !this.renderer.getContext || !this.renderer.getContext()) {
      throw new Error("No se pudo crear el contexto WebGL para Ballpit.");
    }
...
const Ballpit = ({ className = "", followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // Pequeño delay: evita crear un contexto WebGL nuevo en el mismo
    // canvas justo después de que Strict Mode (dev) desmontó/remontó
    // el efecto y forceContextLoss() todavía no liberó el contexto anterior.
    const timeoutId = setTimeout(() => {
      if (cancelled || !canvasRef.current) return;
      try {
        spheresInstanceRef.current = createBallpit(canvasRef.current, { followCursor, ...props });
      } catch (err) {
        console.warn("Ballpit: no se pudo inicializar WebGL, se omite el efecto.", err);
      }
    }, 50);
