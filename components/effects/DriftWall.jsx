...
const DriftWall = ({
  ...
  const hitTestFrameRef = useRef(0);
  ...
  useEffect(() => {
    const animate = (ts) => {
      ...
      // Hit-testing cada 2 frames
      hitTestFrameRef.current += 1;

      if (lastPointerEventRef.current && hitTestFrameRef.current % 2 === 0) {
        const { x, y } = lastPointerEventRef.current;
  ...
  const handlePointerMove = useCallback(
    (e) => {
      ...
      // Solo guardamos la posición.
      // El hit-testing se hará dentro del requestAnimationFrame.
